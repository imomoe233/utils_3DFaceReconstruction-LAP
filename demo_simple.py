...
class Demo():
    def __init__(self, args):
        ...
        if self.detect_human_face:
            ...
            self.face_detector = MTCNN(select_largest=True)
    ...
    def render_animation(self):
        ...
        '''
    def save_results(self, save_dir, i_num):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', default='/home/darui/code_xwd/datasets/Celeb-synthesis-face-align', type=str, help='Path to the directory containing input images')
    parser.add_argument('--result', default='/home/darui/code_xwd/datasets/Celeb-synthesis-face-align_lap', type=str, help='Path to the directory for saving results')
    ...
    for f in sorted(os.listdir(input_dir)):
        if is_image_file(f):
            ...
            image_info_list.append((image_path, f))
    ...
    for im_path, filename in image_info_list:
        if i_num <= -1:
            i_num += 1
            continue
        print(f"[ {i_num} / {num} ] Processing {im_path}")
        pil_im = Image.open(im_path).convert('RGB')
        txt = im_path.split("\\")
        print('/home/darui/code_xwd/datasets/Celeb-synthesis-face-align_lap\\'+txt[-1][:-4])
        if os.path.exists('/home/darui/code_xwd/datasets/Celeb-synthesis-face-align_lap\\'+txt[-1][:-4]):
            print('skip'+im_path)

        '''
        try:
            face = model.detect_face(pil_im)
            face1 = Image.fromarray(face)
        except:
            #print("Could not detect faces in the image")
            continue
        face2 = face1.resize((1024, 1024))
        face2.save(f'E:\code_xwd\dataset/temp/{filename[:-4]}.jpg')
        '''
        '''
        image_tensor = torch.from_numpy(np.asarray(pil_im)).unsqueeze(0)
        x = coach.run(image_tensor)
        torch.save(x, result_dir+'/'+filename[:-4])
        '''
        try:
            result_code = model.run(pil_im)
        except:
            os.remove(im_path)
            print('Delete:' + im_path)
            continue
        if result_code == -1:
        ...
        days, hours, minutes, seconds = seconds_to_dhms(elapsed_time)
        print(f"程序执行时间: {days} 天, {hours} 小时, {minutes} 分钟, {seconds} 秒")

        save_dir = os.path.join(result_dir, os.path.splitext(os.path.basename(im_path))[0])
        model.save_results(save_dir, i_num)
